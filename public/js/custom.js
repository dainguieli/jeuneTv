...
(function (jQuery) {
	...
	jQuery(document).ready(function() {
		...
		----------------------------------------------------------------------- */
		jQuery("#load").fadeOut();
		jQuery("#loading").delay(2500).fadeOut("slow");
...
const carousel=document.jQuerySelector('.carousel');
let slider=[];
...
const createSlide=()=>{
	...
	h1.className='movie-des'

	sliders.push(slide);
if (sliders.length) {
	slider[0].style.marginLeft=`calc(-${100 * (sliders.length - 2)} - ${30*(slide.length - 2)}px)`
}
	for (let i = 0; i < 3; i++) {
		createSlide();
